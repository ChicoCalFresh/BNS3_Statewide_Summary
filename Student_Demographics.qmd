...
The Statewide survey captures students of all class standing. Of the `r sum(table(bns$q15))` respondents, there were `r unname(table(tmp)[1])` freshmen, `r unname(table(tmp)[2])` sophomores,
`r unname(table(tmp)[3])` juniors, and `r unname(table(tmp)[4])` seniors.
...
The majority of students in the data seem to be majoring in some type of `r names(sort(table(bns$q16), decreasing=TRUE)[1])` program (`r unname(sort(table(bns$q16), decreasing=TRUE)[1])` people). `r unname(sort(table(bns$q16), decreasing=TRUE)[2])` of the students are `r names(sort(table(bns$q16), decreasing=TRUE)[2])` majors.
