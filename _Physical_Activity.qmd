...
tmp %>%
  kable() %>% kable_styling(latex_options = c("HOLD_position", "striped", "bordered", "hover"))  %>%  
    row_spec(1, bold = T, color = "white", background = chc_blue) 
...
-   The majority of participants claim to have done `r names(sort(table(bns$q83_1), decreasing = TRUE))[1]` days of moderate exercise in the past week (`r unname(sort(table(bns$q83_1), decreasing = TRUE))[1]` people). The general trend is that as the number of days increases, the number of students who exercised for those given number of days decreases. This seems to be the most commonly performed type of exercise from the graph above (typically the highest number of days per week out of all exercise types).
