...
This page looks at the subset of the 2,110 EOP/EOPS students that were determined to be likely eligible for CalFresh based on self-reported household size and income (see [CalFresh Food Eligibility](CalFresh.qmd#calfresh-food-eligibility) for more information). Of the 1341 students who were likely eligible for CalFresh, 1289 responded to a survey question inquiring about their knowledge/use of CalFresh benefits and `r chcRne::count_and_percent(df_cf_eligible, q46_1, values = c("Used it before attending college", "Heard of it, but never used it", "Used it in the past as a college student", "Never heard of it"))` did not report currently using CalFresh as a college student. 55 of the 1341 students who were likely eligible for CalFresh either did not finish the survey or selected "Prefer not to answer" for this question.

See [Information on Missing Data](index.qmd#ref-md)

## College Campus
```{r}
#| fig-height: 6
#| fig-width: 8
#| label: fig-campus-breakdown
#| fig-cap: !expr sprintf("Survey Participants by Campus %s.", chcRne::print_n_reporting(df_cf_eligible, "school"))
tmp2 <- tmp %>% select(school2, sector, sector_color) %>% na.omit()

school_data <- tmp2 %>% 
  group_by(school2, sector) %>% 
  summarize(
    count = n(),
    npct = paste0(count, " (", unname(formattable::percent(count/nrow(tmp), digits = 1)), ")")
  )

school_data %>% ggplot(aes(x = school2, y = count, fill = sector)) +
  geom_col() + geom_text(aes(label = npct), hjust = -0.05) + 
  coord_flip() + ylim(0, 250) + theme_bw() + xlab("School") + ylab("Number of Students") + scale_fill_manual(values = c('gold', '#9D2235', '#4895EF'), name = 'School Types', labels = c('CCC', 'CSU', 'UC'))
```
...
**Summary: ** A total of `r chcRne::count_and_percent(df=tmp, var=sector, values="CSU")`, `r chcRne::count_and_percent(df=tmp, var=sector, values="CCC")` and `r chcRne::count_and_percent(df=tmp, var=sector, values="UC")` of likely CalFresh eligible college students were enrolled in CSU, CCC and UC campuses, respectively. All college students were either in the Educational Opportunity Program (EOP) at the UC or CSU systems or in the Extended Opportunity Programs and Services (EOPS) in the CCC systems.

<br/>

## Students Likely Eligible but Not Receiving CalFresh
```{r}
#| fig-height: 6
#| fig-width: 8
#| label: fig-likely-eligible-not-receiving
#| fig-cap: !expr sprintf("Survey Participants Likely Eligible for, but not Receiving CalFresh Benefits by Campus %s.", chcRne::print_n_reporting(df_cf_eligible, "CalFresh"))
...
summarized_data |> ggplot(aes(x = school2, y = pct, fill = sector)) +
  geom_col() + geom_text(aes(label = npct), hjust = -0.05) + 
  coord_flip() + theme_bw() + xlab("School") + ylab("Students Likely Eligible but Not Receiving CalFresh (%)") +
...
* *The CalFresh eligibility variable was derived using self-reported household size and income (See [CalFresh Food Eligibility](CalFresh.qmd#calfresh-food-eligibility)*)
...
**Additional Notes: ** Students who selected "Currently use as a college student" were marked as currently using CalFresh. Students who selected "Never heard of it", "Heard of it, but never used it", "Used it in the past as a college student", or "Used it before attending college" were marked as not receiving CalFresh.
...
*Note: The below plots are looking at the subset of 836 students that were likely eligible for CalFresh, but not currently using it at the time of the survey.*


## Age 
```{r}
#| label: fig-age
#| fig-cap: !expr sprintf("Reported Age %s.", chcRne::print_n_reporting(df_eligible_not_using, "age"))
...
```{r}
#| label: fig-gender
#| fig-cap: !expr sprintf("Gender Identity %s.", chcRne::print_n_reporting(df_eligible_not_using, "q7"))
#Gender
...
```{r}
#| label: fig-ethnicity
#| fig-cap: !expr sprintf("Ethnic/Hispanic Identity %s.", chcRne::print_n_reporting(tmp, "hispanic"))
...
```{r}
#| label: fig-firstgen
#| fig-cap: !expr sprintf("First-Generation Students %s.", chcRne::print_n_reporting(tmp3, "firstgen"))
...
```{r}
#| label: tbl-barriers
#| tbl-cap: "Top 10 Reported Barriers to Using CalFresh for Students who are Likely Eligible but not using CalFresh."
...
```{r}
#| label: tbl-other
#| tbl-cap: "Text Responses for Students who Selected 'Other'"
...
**Additional Notes: ** For students who selected 'Other,' we have also included the responses they provided.
