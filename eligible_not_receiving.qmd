...
Robin wants to remove this question - see additional notes in next question
```{r}
#| fig-height: 6
#| fig-width: 8
...
**Summary: ** A total of `r chcRne::count_and_percent(df=tmp, var=sector, values="CSU")`, `r chcRne::count_and_percent(df=tmp, var=sector, values="CCC")` and `r chcRne::count_and_percent(df=tmp, var=sector, values="UC")` of likely CalFresh eligible college students were enrolled in CSU, CCC and UC campuses, respectively. 

<br/>

## Proportion at each campus not receiving CalFresh
The distribution of student participation for these  these 836 likely eligible but not receiving - ios the same as thje overall distribution of all participating students from each of the 12 campuses (with clovis and butte swapped in position) then add direct link to  https://chicocalfresh.github.io/BNS3_Statewide_Summary/Academic_Profile.html#college-campus 
...
> robin wants to remove these notes since the %missing is essentially the same as the overall sample
