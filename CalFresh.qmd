...
Students were asked to report information relating to CalFresh and CalFresh eligibility. This includes U.S. citizenship (or lawful permanent residency), household size, and monthly household income. Those that were not using CalFresh were asked to report any barriers to applying for CalFresh, while those that reported currently using CalFresh were asked to report their monthly Electronic Benefit Transfer (EBT), whether the EBT received is enough for them, and the perceived impact of CalFresh. 
...
## CalFresh Status {#cal_status_reference}


```{r}
#| fig-height: 2
#| fig-width: 8
#| fig-cap: !expr sprintf("%s CalFresh Status %s", NextFigure(), print_n_reporting('q46_1'))
...
**Summary:** A total of `r get_count_and_percent(bns$q46_1, "Currently use as a college student")` students reported currently using CalFresh, `r get_count_and_percent(bns$q46_1, "Used it in the past as a college student")` reported they used CalFresh in the past as a college student, and `r get_count_and_percent(bns$q46_1, "Used it before attending college")` reported that they used CalFresh before attending college. Additionally, `r get_count_and_percent(bns$q46_1, "Heard of it, but never used it")` students reported that they heard of CalFresh, but never used it, while `r get_count_and_percent(bns$q46_1, "Never heard of it")` reported that they never heard of it. 
...
::: col-md-6
```{r}
#| fig-cap: !expr sprintf("%s CalFresh Eligibility %s", NextFigure(), print_n_reporting('calfresh_eligible'))
...
**Additional Notes: ** Only county workers have the authority to determine official eligibility for CalFresh. These eligibility rates were estimated based on self-reported household size and income. Below is a list of eligibility criteria for students:  

- Meets household size/income requirements from the [Department of Social Services](https://www.cdss.ca.gov/inforesources/cdss-programs/calfresh/eligibility-and-issuance-requirements#income).
...
**Summary:** A total of `r get_count_and_percent(bns$calfresh_eligible, "1")` students were likely eligible for CalFresh while `r get_count_and_percent(bns$calfresh_eligible, "0")` were likely not eligible. Many students selected "Prefer not to answer" when reporting their income. Calfresh eligibility was unable to be determined among these students and among any students who answered "Prefer not to answer" to household size.  
...
## CalFresh Eligibility Breakdown


```{r}
#| tbl-cap: !expr sprintf("%s CalFresh Eligibility Breakdown", NextTable())
...
Only students who reported currently using CalFresh or used it in the past as a college student saw the following set of questions. Students were asked to report how much EBT they received per month, how many people they fed with their EBT, whether they received enough money, how long their EBT lasts every month, and their perceived impact of CalFresh on academic and non-academic matters.   


## CalFresh EBT {#reference2}


```{r}
#| fig-cap-location: top
#| fig-cap: !expr sprintf("%s CalFresh Application Household Size %s & %s Number of People Fed Through EBT %s", NextFigure(), print_n_reporting('q47'), NextFigure(), print_n_reporting('fed_through_EBT'))
#| fig-width: 9
p1 <- plot_frq(bns$q47, geom.colors = plot_color, wrap.title = 40, ylim=c(0,500), wrap.labels = 15) +
  xlab('\nDid you apply for CalFresh EBT as an individual \nor with other people in your household?')
...
**Summary:** The average amount of money received per household using CalFresh (denoted by the dotted line) was \$`r round(mean(bns$q49_1, na.rm=TRUE),2)`, while the median was \$`r round(median(bns$q49_1, na.rm=TRUE),2)`. The amounts reported ranged from \$`r min(bns$q49_1, na.rm=TRUE)` to \$`r max(bns$q49_1, na.rm=TRUE)`.
...
- *For the number of people fed through EBT, see [CalFresh EBT](#reference2)*
- *For the amount of EBT received per month, see [Monthly CalFresh Money](#reference3)*
...
```{r}
#| fig-cap: !expr sprintf("%s Perceived Impact of CalFresh", NextFigure())
#| fig-height: 8
#| fig-width: 8.5
...
**Survey Questions:** *For the following questions, please choose how much you agree or disagree with the impact CalFresh has or had on your life. While you received CalFresh benefits:*

**Summary:** Because students received CalFresh, `r get_count_and_percent2(bns$q52_3, "Agree", "Strongly agree")` reported they had more money for bills, `r get_count_and_percent2(bns$q52_7, "Agree", "Strongly agree")` reported purchasing healthier foods, and `r get_count_and_percent2(bns$q52_8, "Agree", "Strongly agree")` reported cooking more frequently. Only `r get_count_and_percent2(bns$q52_1, "Agree", "Strongly agree")` students reported that they worked fewer hours because of CalFresh, `r get_count_and_percent2(bns$q52_5, "Agree", "Strongly agree")` shopped more frequently at the farmers' market, and `r get_count_and_percent2(bns$q52_6, "Agree", "Strongly agree")` reported learning more about healthier foods. 
...
```{r}
#| tbl-cap: !expr sprintf("%s Reported Barriers to Using CalFresh", NextTable())
...
cf_barrier_vars %>%
select("Reasons for Not Using CalFresh" = xlab, "Yes (%)" = pct_lab) %>%
kable() %>% kable_styling(bootstrap_options = "striped") %>%  column_spec(2, width='3.5cm')

```

**Survey Questions:** *What are the reasons you do not have CalFresh or have never used CalFresh? (Select all that apply)*

**Additional Notes:** Only students who reported never using Calfresh as a college student (see [CalFresh Status](#cal_status_reference)) saw this question.


**Summary:** The most common reason that students reported not using CalFresh was because they do not think they are eligible where `r get_count_and_percent(bns$calfresh_barrier_do_not_think_eligible, "1")` students selected this option. Meanwhile `r get_count_and_percent(bns$calfresh_barrier_other_students_need, "1")` students reported that they felt other students need CalFresh more than they do, and `r get_count_and_percent(bns$calfresh_barrier_no_time_app, "1")` reported that they have no time to complete the application.
