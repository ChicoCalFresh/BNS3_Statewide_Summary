...
p1 <- plot_frq(bns$q47, geom.colors = plot_color, wrap.title = 40, ylim=c(0,500), wrap.labels = 15, 
  ...
  xlab('')
p2 <- plot_frq(bns$fed_through_EBT, geom.colors = plot_color, wrap.title = 40, title="How many people in the household are regularly fed through your CalFresh EBT card?") + xlab('')
...
```{r}
#| fig-width: 6
p1 <- plot_frq(bns$q50, geom.colors = plot_color, ylim=c(0,200), title="I feel that the amount that is added to my CalFresh EBT card each month is enough money to provide healthy food for the month.") + xlab('')
p2 <- plot_frq(bns$q51, geom.colors = plot_color, ylim=c(0, 210), wrap.labels = 12, title="When do you typically use up the balance on your CalFresh EBT card?") + xlab('')
