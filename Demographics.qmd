...
plot_frq(rec$q7, sort.frq="desc", geom.colors = plot_color) + xlab('Gender') + ylab("Frequency")
...
plot_frq(tmp$hispanic, geom.colors = plot_color, ylim=c(0, 1800)) + xlab("") + ylab("Frequency")
```
:::
...
#| fig-height: 4
#| fig-width: 6
plot_frq(bns$q11, geom.colors = plot_color,) + xlab('Number of Dependents') + ylab("Frequency")
```
...
plot_frq(tmp3$firstgen, geom.colors = plot_color, ylim=c(0, 2300)) + xlab('') + ylab("Frequency")
```
:::
...
plot_frq(bns$q16, geom.colors = plot_color, ylim=c(0,875), wrap.labels = 16, sort.frq = 'desc') + xlab('') + ylab("Frequency") + theme(axis.text.x = element_text(size = 10))
...
plot_frq(bns$gpa_fac, geom.colors = plot_color) + xlab('GPA') + ylab("Frequency")
```
...
plot_frq(bns$units_binned, geom.colors = plot_color) + xlab('Units') + ylab("Frequency")
```
