...
```{r}
tmp <- bns %>% 
  mutate(q11 = ifelse(bns$q11 == 0, "I don't have dependents", bns$q11),
         q11 = as.numeric(q11))
```
...
Summary: `r get_count_and_percent(bns$firstgen, '1')` of all students report first generation status based on their parent/guardian education level. This outcome is not surprising because the students were either enrolled in EOP or EOPS and both programs recruit students with first generation status. 1697 (80.4%) of students reported that their parents/guardians had less than an Associate’s degree, and 719 (42.3%) of those students reported their parents/guardians did not have a high school diploma or GED. 
