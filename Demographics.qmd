...
Students’ personal demographics (e.g. age, ethnicity, number of dependents, etc.) were obtained and visualized in this section. Specific survey questions and summaries are found below each figure. As a reminder, students had the option to select “Prefer not to answer” for all questions, and these responses were set as missing values. In addition, `r get_count_and_percent(bns$finished, 'FALSE')` students did not finish the survey. The n count for every survey question reflects the total number of non-missing responses while the percentage reflects the percent of non-missing responses. 
...
**Summary: ** The average age of the students was `r round(mean(bns$age, na.rm = TRUE),1)` years with a range of `r min(bns$age, na.rm = TRUE)` to `r max(bns$age, na.rm = TRUE)` and the majority falling between 20-30 years old. 
...
#| fig-width: 6
#| label: fig-q11
#| fig-cap: !expr sprintf("Reported dependents by college students %s", print_n_reporting('q11'))
...
**Additional Notes: ** Approximately `r get_count_and_percent(bns$q11, "I don't have dependents")` students reporting no dependents and `r get_count_and_percent(bns$q11, "0")` reporting dependents with less than 50% financial support were omitted from the chart. 
...
#| fig-width: 7.5
#| label: fig-q13
#| fig-cap: !expr sprintf("Parent/guardian education level reported by college students %s", print_n_reporting('q13'))
...
**Summary: ** Approximately `r get_count_and_percent(bns$firstgen, '1')` students reported first-generation status based on their parent/guardian education level. This outcome is not surprising because the students were either enrolled in EOP or EOPS and both programs recruit students with first-generation status. A total of `r count_and_percent(bns$q13, "Less than high school")` students reported their parents/guardians did not have a high school diploma or GED while `r count_and_percent(bns$q13, "High school diploma or GED")` reported the highest level of education completed by their parents is high school. 
...
#| fig-width: 4
#| label: fig-firstgen
#| fig-cap: !expr sprintf("Reported First-Generation Status by College Students %s", print_n_reporting('firstgen'))
...
::: col-md-5
**Survey Question: ** *This variable was derived from the question in @fig-q13*
