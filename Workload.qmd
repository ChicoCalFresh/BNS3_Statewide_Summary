...
Respondents stated a wide range of paid hours per week, with 50% of them reporting between <insert IQR here> hours, with an average of X and median of X. 

The majority of survey participants work either part-time or not at all. Only a small number `r NROW(bns$paid_work_hours >= 40)` of participants are considered to be working full-time (40 or more hours per week). Below is a breakdown of the exact paid hours worked by survey participants.
...
The data indicates that the majority of survey participants (`r table(bns$bin)['1']`) do spend some time each week caring for family members. A minority (`r unname(table(bns$caregiving_hours)['0'])`) of participants estimated that they spent 0 hours per week for family obligations. `r unname(table(bns$caregiving_hours > 40)[2])` participants reported 40 or more hours per week on these family obligations. Below is a breakdown of these unpaid hours spent on family obligations each week.
