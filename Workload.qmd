...
Students reported a wide range of paid hours per week, with 50% of them reporting between `r quantile(bns$paid_work_hours, na.rm = TRUE, .25)` and `r quantile(bns$paid_work_hours, na.rm = TRUE, .75)`  hours, with an average of `r round(mean(bns$paid_work_hours, na.rm = TRUE), digits = 2)` and median of `r median(bns$paid_work_hours, na.rm = TRUE)`. 

The majority of students work either part-time or not at all. Only a small number `r NROW(bns$paid_work_hours >= 40)` of students are considered to be working full-time (40 or more hours per week). Below is a breakdown of the exact paid hours worked by students.
...
```{r}
#| fig-width: 8
plot_frq(bns$q20, geom.colors = plot_color, ylim=c(0,700), wrap.labels = 19, sort.frq = 'desc') + xlab('') + ylab("Frequency")
```
...
We can see that the majority of students (`r unname(table(bns$unpaid_work_hours)['0'])`) do not work an unpaid job. A small number of students work part-time at an unpaid job and only a select few work full-time at an unpaid job. Below is a breakdown of the number of unpaid hours worked per week.
...
**Below is a combination of both paid and unpaid hours worked by students to get a further understanding of the workload students face on a weekly basis. `r print_n_reporting('Nonacademic')`**
