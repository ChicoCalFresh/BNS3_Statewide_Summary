...
The majority of students (`r unname(table(bns$unpaid_work_hours)['0'])`) do not work an unpaid job. A small number of students work part-time at an unpaid job and only a select few work full-time at an unpaid job. Below is a breakdown of the number of unpaid hours worked per week. [Figure 2. @fig-unpaid_work_hours]
