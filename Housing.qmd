...
#| fig-height: 3.5
#| fig-width: 4.5
plot_frq(bns$q24, geom.colors = plot_color, ylim=c(0,1800)) + xlab('') + ylab("Frequency")
```
:::
...
The majority of students (`r unname(table(bns$housing_barrier_none))[2]`) reported not experiencing barriers to housing in the last 12 months. Among the students who did experience barriers to housing, lack of deposit funds (`r unname(table(bns$housing_barrier_deposit_funds))['1']` students\`) was most common.
...
**In the past 12 months, how many times have you been unsure of where you were going to sleep at night?**

```{r}
#| fig-height: 4.3
#| fig-width: 4.5

plot_frq(bns$q34, geom.colors = plot_color, wrap.title=40, wrap.labels =12, ylim=c(0,1850)) + xlab('') + ylab("Frequency")
```
...
#| fig-height: 5
#| fig-width: 9
plot_frq(bns$q37, geom.colors = plot_color, wrap.labels = 17, ylim=c(0,1600), sort.frq = 'desc') + xlab('') + ylab("Frequency") + theme(axis.text.x = element_text(size = 10))
```
...
#| fig-height: 3
#| fig-width: 4
plot_frq(bns$q23, geom.colors = plot_color, ylim=c(0,2150)) + xlab('') + ylab("Frequency")
```
:::
